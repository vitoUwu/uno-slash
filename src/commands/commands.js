      ...
      if (subCommand === "single") {
        ...
        if (!command)
          return interaction.reply({
            embeds: [error("Comando não encontrado")],
          });
        ...
        if (guildId && !guild)
          return interaction.reply({ embeds: [error("Servidor Inválido")] });
        ...
        await client.application.commands
          ...
          .then(() =>
            interaction.editReply({ embeds: [success("Comando Implementado")] })
          )
          .catch((err) => {
            interaction.editReply({
              embeds: [
                error(
                  `Ocorreu um erro ao implementar comando\n\`\`\`${err.stack}\`\`\``
                ),
              ],
            });
        ...
        client.commands
          ...
          .forEach((cmd) => {
            if (!!cmd.slashExecute)
              commands.push(
                transformCommand(cmd, ApplicationCommandType.ChatInput)
              );
            if (!!cmd.userContextExecute)
              commands.push(transformCommand(cmd, ApplicationCommandType.User));
            if (!!cmd.messageContextExecute)
              commands.push(
                transformCommand(cmd, ApplicationCommandType.Message)
              );
          });
        ...
        await client.application.commands
          ...
          .then(() =>
            interaction.editReply({
              embeds: [success("Comandos Implementados")],
            })
          )
          .catch((err) => {
            interaction.editReply({
  ...
  async autocompleteExecute(interaction) {
    ...
    if (focused.name === "command") {
      ...
      data = client.guilds.cache
        .filter(
          (guild) =>
            guild.name.startsWith(focused.value) ||
            guild.id.startsWith(focused.value)
        )
        .map((guild) => ({ name: guild.name, value: guild.id }))
        .slice(0, 25);
    } else {
      data = [
        { name: "Unsupported Autocomplete", value: "Unsupported Autocomplete" },
      ];
    }
