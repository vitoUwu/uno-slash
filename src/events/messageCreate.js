...
module.exports = {
  ...
  execute(message) {
    ...
    if (!command || (command.ownerOnly && message.author.id !== ownerId))
      return;
    ...
    if (userCooldown)
      return message.reply({
        embeds: [
          error(
            `Você está usando os comandos muito de pressa! Espere mais \` ${(
              (Date.now() - userCooldown) /
              1000
            ).toFixed(1)} \` segundos`
          ),
        ],
    ...
    cooldowns.set(
      `${commandName}_${message.author.id}`,
      Date.now() + command.cooldown * 1000
    );
    setTimeout(
      () => cooldowns.delete(`${commandName}_${message.author.id}`),
