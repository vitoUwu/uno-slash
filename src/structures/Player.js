  ...
  async playCard(id, interaction) {
    ...
    if (!game.started)
      return await interaction.reply({
        embeds: [error(locales(interaction.locale, "player.notStarted"))],
      });
    ...
    const index = this.cards.findIndex(
      (c) =>
        c === id ||
        game.parseCardId(c).toString().toLowerCase() === id.toLowerCase()
    );
    if (index < 0)
      return await interaction.reply({
        embeds: [error(locales(interaction.locale, "player.cardNotFound"))],
        ephemeral: true,
      });
    ...
    if (game.stackedCombo && serializedCard.number !== "+2") {
      await interaction.reply({
        embeds: [error(locales(interaction.locale, "player.only+2"))],
        ephemeral: true,
      });
      return;
    }
    ...
    if (
      !this.compatibleColor(serializedLastCard, serializedCard) &&
      !this.compatibleNumber(serializedLastCard, serializedCard)
    ) {
      await interaction.reply({
        embeds: [error(locales(interaction.locale, "player.invalidCard"))],
        ephemeral: true,
      });
      return;
    }
    ...
    if (cardType === "special") {
      ...
      if (cardNumber === "+4") {
        ...
        game.message.push({
          ...
          variables: [
            interaction.member,
            game.whoPlaysNext.member,
            game.whoPlaysNext.cards.length,
          ],
        });
        game.pushPlayer();
      } else {
