...
module.exports = class Game {
  ...
  parseCardId(cardId) {
    ...
    return {
      ...
      toString: () =>
        `${emoji} ${
          !color
            ? locales(this.locale, "game.cards.wild")
  ...
  giveCards(player, amount) {
    ...
    for (let i = 0; i < amount; i++)
      c.push(cards[(cards.length * Math.random()) | 0]);
    player.cards.push(...c);
  }
  ...
  async announceError(err) {
    ...
    this.send({ content, flags: "SuppressEmbeds" }).catch((err) =>
    ...
    this.timeout = setTimeout(() => {
      ...
      this.message.push({
        ...
        variables: [
          ...
          this.whoPlaysNow.cards.length,
          this.stackedCombo || 2,
        ],
      });
      this.stackedCombo = 0;
      this.whoPlaysNow.skippedRounds++;
      if (this.whoPlaysNow.skippedRounds >= 2)
        this.removePlayer(this.whoPlaysNow.id);
        ...
        embeds: [
          {
            description: locales(
              this.locale,
              "game.embeds.end.descriptions.inactivity"
            ),
        ...
        embeds: [
          {
            description: `${locales(
              ...
              .map(
                (w, i) =>
                  `[${this.getRankingPositionEmoji(i)}] #${i + 1} | ${w}`
              )
              .join("\n")}\`\`\``,
            color: Colors.Blurple,
            footer: { text: locales(this.locale, "game.embeds.end.footer") },
  ...
  pushWinner(id) {
    ...
    if (index < 0) return;
    this.winners.push(
      ...this.players.splice(index, 1).map((p) => p.member.user.username)
    );
  }
  ...
  async nextPlayer(interaction, uno) {
    ...
    const row = new ActionRowBuilder().setComponents([
      ...
      new ButtonBuilder()
        .setCustomId("report_uno")
        .setLabel(locales(this.locale, "game.report"))
          ...
          description: `${
            this.message.length
              ? `${this.message.map((msg) =>
                  locales(this.locale, msg.key, ...msg.variables)
                )}\n\n`
              : ""
          }${locales(
            this.locale,
            ...
            .map(
              (player, index) =>
                `#${index + 1} | ${player.member.user.username}: ${
                  player.cards.length
    ...
    if (uno) {
      ...
      collector.on("collect", async (i) => {
        if (i.customId === "uno" && i.user.id === this.lastPlayer.id) {
          collector.stop();
          await reply.edit({ components: [] }).catch((err) => {
            this.client.logger.error(err);
            this.announceError(err);
          });
        }
