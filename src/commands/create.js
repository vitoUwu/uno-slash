...
module.exports = {
  ...
  async slashExecute(interaction) {
    ...
    const row = new ActionRowBuilder().addComponents(
			new ButtonBuilder().setCustomId("join").setLabel(locales(interaction.guildLocale, "commands.create.joinMatch")).setStyle(ButtonStyle.Primary),
			new ButtonBuilder().setCustomId("start").setLabel(locales(interaction.guildLocale, "commands.create.startMatch")).setStyle(ButtonStyle.Success)
		);

    const reply = await interaction.reply({
			embeds: [{
        description: `${locales(interaction.guildLocale, "commands.create.matchQueueDescription")}\n\n${locales(interaction.guildLocale, "commands.create.players")}:\`\`\`${game.players.map((p) => p.member.user.username).join("\n")}\`\`\``,
		...
		collector.on("collect", async (i) => {
			if (i.customId === "start") {
				if (i.user.id !== game.authorId) return await i.reply({ embeds: [error(locales(i.locale, "commands.create.youCantStart"))], ephemeral: true });
			...
			if (i.customId === "join") {
				if (game.players.some((player) => player.member.id === i.user.id)) return await i.reply({ embeds: [error(locales(i.locale, "commands.create.alreadyJoinedMatch"))], ephemeral: true });
		...
		collector.on("end", async (_, reason) => {
			if (reason === "time") {
				await reply.edit({ embeds: [error(locales(interaction.guildLocale, "commands.create.cancelledByInactivity"))], components: [] });
				return;
			}
		});
  }
