  ...
  async slashExecute(interaction) {
    ...
    if (
      ...
      return await interaction.editReply({
        embeds: [
          error(locales(interaction.locale, "commands.create.invalidChannel")),
        ],
        ephemeral: true,
      });
    ...
    if (client.games.get(interaction.channelId))
      return await interaction.editReply({
        embeds: [
          error(
            locales(interaction.locale, "commands.create.alreadyStartedMatch")
          ),
        ],
        ephemeral: true,
      });
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("join")
        .setLabel(locales(interaction.locale, "commands.create.joinMatch"))
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId("start")
        .setLabel(locales(interaction.locale, "commands.create.startMatch"))
        .setStyle(ButtonStyle.Success),
      new ButtonBuilder()
        .setCustomId("cancel")
        .setLabel(locales(interaction.locale, "commands.create.cancelMatch"))
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "join") {
        if (game.players.some((player) => player.member.id === i.user.id))
          return await i.reply({
            embeds: [
              error(locales(i.locale, "commands.create.alreadyJoinedMatch")),
            ],
            ephemeral: true,
          });
