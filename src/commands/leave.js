...
module.exports = {
	...
	async slashExecute(interaction) {
		...
		if (!game) return await interaction.reply({ embeds: [error(locales(interaction.locale, "commands.leave.noMatchs"))] });

		const player = game.getPlayer(interaction.user.id);
		if (!player) return await interaction.reply({ embeds: [error(locales(interaction.locale, "commands.leave.notParticipating"))], ephemeral: true });
