...
module.exports = {
   ...
   */
    ...
    const { client } = interaction;
		...
		const game = client.games.get(interaction.channelId);
		if (!game) return await interaction.respond([{ name: locales(interaction.locale, "commands.play.noMatchs"), value: locales(interaction.locale, "commands.play.noMatchs") }]);

		if (!game.started) return await interaction.respond([{ name: locales(interaction.locale, "commands.play.notStarted"), value: locales(interaction.locale, "commands.play.notStarted") }]);

		const player = game.getPlayer(interaction.member.id);
		if (!player) return await interaction.respond([{ name: locales(interaction.locale, "commands.play.notParticipating"), value: locales(interaction.locale, "commands.play.notParticipating") }]);

		const value = interaction.options.getFocused(true).value.toLowerCase();

		let data = player.cards.filter((cardId) => cardId.includes(value) || game.parseCardId(cardId).toString().toLowerCase().includes(value)).map((card) => ({ name: game.parseCardId(card).toString(), value: card }));

		if (!data.length) data = [{ name: locales(interaction.locale, "commands.play.cardNotFound"), value: locales(interaction.locale, "commands.play.cardNotFound") }];
